	...
	printOrderByOrderId: async (orderid) => {
		return new Promise(async (resolve, reject) => {
			try {
				...
				const userDetail = orderDetail.userDetail;
				const shopDetail = orderDetail.shopDetail;
				let orderInfo = '';
				...
				if (orderDetail.order_type === 1) {
					...
					if (goods.length === 0) {
						...
						goods.forEach((item) => {
							const itemMoney = Number(item.price * item.num).toFixed(2);
					...
					orderInfo += `是否加急：${orderDetail.urgency === 2 ? '加急订单' : '普通订单'}<BR>`;
				...
				if (orderDetail.order_type === 3) {
					...
					orderInfo += '-------------------------------<BR>';
					orderInfo += `订单编号：${orderDetail.code}<BR>`;
					orderInfo += `兑换物品：${goodDetail.name}<BR>`;
				...
				if (orderDetail.order_type === 5) {
					...
					orderInfo += `应付金额：${orderDetail.payMoney} 元<BR>`;
					orderInfo += `备注：${orderDetail.desc || '无'}<BR>`;
					orderInfo += `配送方式：${Number(orderDetail.send_status) === 1 ? 'MOVING洗衣柜' : '用户自取'}<BR>`;
					orderInfo += `下单时间: ${moment().format('YYYY-MM-DD HH:mm:ss')}<BR><BR>`;
					orderInfo += '<QR>MOVING</QR>';
				}
				// 打印机编号
